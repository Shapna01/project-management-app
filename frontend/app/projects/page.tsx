...
export default function ProjectsPage() {
  ...
  const COLORS = ["#2F8F9D", "#6C63FF", "#3FA7D6", "#E53935"];
 ...
 const getProjectStats = (projects: Project[]) => {
    const stats = { completed: 0, onhold: 0, inprogress: 0, pending: 0 };

    projects.forEach((p) => {
      if (p.status === "completed") stats.completed++;
      else if (p.status?.includes("hold")) stats.onhold++;
      else if (p.status?.includes("progress")) stats.inprogress++;
      else stats.pending++;
    });
...
const renderLabel = ({
  ...
  <div className="flex">
    ...
    <div className="flex-1 ml-64 mt-16 p-6">
      <h1 className="text-2xl font-semibold mb-6 text-black">Projects</h1>
        ...
        <div className="col-span-2 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm flex items-center justify-between h-[260px]">
              <h2 className="text-sm font-semibold text-black">Project</h2>
  ...
  <Cell key={i} fill={COLORS[i]} />
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm h-[260px] flex flex-col justify-between">
              ...
              <LineChart width={350} height={170} data={lineData}>
                ...
                <Line dataKey="count" stroke="#FF6B6B" strokeWidth={2} dot={false} />
                <Line dataKey="target" stroke="#6C63FF" strokeWidth={2} dot={false} />
              </LineChart>
            </div>
 ...
 <div
  className={`grid grid-cols-6 gap-4 pr-2 transition-all duration-300 ${
    showAllDevs
      ? "max-h-[300px] overflow-y-auto"
      : "max-h-[180px] overflow-hidden"
  }`}
>
    {(showAllDevs ? projects : projects.slice(0, 18)).map((_, i) => (
      <div
        key={i}
        className="w-12 h-12 rounded-full overflow-hidden bg-gray-200 hover:scale-105 transition"
      >
        <img
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm flex flex-col h-[540px]">
          <div className="flex justify-between mb-4">
